function get(socket,data,fn){
	...
	//returnFn();
	//return;
	data_mg.updateTime.find({"parentKey":"com"},function(err,doc){
		if(err){
			...
			if(doc&&doc.length&&doc[0].childKey>data.data.time){
				...
				data_mg.com.find({},function(errA,docA){
					if(errA){
						result.success=false;
...
function add(socket,data,fn){
	...
	var newCom=new data_mg.com(data.data);
	newCom.save(function(err,Clientsc){
		...
		if(err){
			...
			returnFn();
							...
							data_mg.updateTime.update({"parentKey":"com"},{$set:{"childKey":new Date().getTime()}},{},function(errB){
								if(errB){
									result.success=false;
...
function remove(socket,data,fn){
	console.log("com/remove");
	//data.data = "ddgdgd"/*管理员id*/
	console.log(data.data);
	var result={code:0};
	var returnFn=function(){
		if(socket){
	 	socket.emit("com_remove",result);
	 }
	 	else if(fn){
	 		var returnString = JSON.stringify(result);
	 		fn(returnString);
